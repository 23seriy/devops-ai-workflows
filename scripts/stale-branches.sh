...
stale_local=0
for branch in $(git for-each-ref --sort=committerdate --format='%(refname:short) %(committerdate:iso8601)' refs/heads/ | while read -r name br_date; do
  branch_epoch=$(date -jf "%Y-%m-%d %H:%M:%S %z" "$br_date" +%s 2>/dev/null || date -d "$br_date" +%s 2>/dev/null || echo 0)
...
if [ "$INCLUDE_REMOTE" = true ]; then
  ...
  stale_remote=0
  git for-each-ref --sort=committerdate --format='%(refname:short) %(committerdate:iso8601)' refs/remotes/origin/ | while read -r name br_date; do
